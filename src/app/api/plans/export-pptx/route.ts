...
import { z } from "zod";
import {
  renderMarketingPlanPptx,
  type MarketingPlanPptxCompany,
  type MarketingPlanPptxPlan,
  type MarketingPlanPptxVideoRelease
} from "@/lib/pptx/marketing-plan";
...
async function logPptxExport(ownerId: string, companyId: string, planId: string) {
...
export async function GET(request: Request) {
  ...
  if (!parsedRequest.success) {
    return NextResponse.json(
      { error: "Invalid PowerPoint export request." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const pptx = await renderMarketingPlanPptx({
      company: company as MarketingPlanPptxCompany,
      plan: plan as MarketingPlanPptxPlan,
      timezone,
    ...
    await logPptxExport(ownerId, plan.company_id, plan.id);

    const month = String(plan.month).slice(0, 7);
    const filename = `campaignova-${slugFilename(company.name) || "plan"}-${month}.pptx`;
    ...
    const message =
      error instanceof Error ? error.message : "Unknown PowerPoint export error";

    return NextResponse.json(
      { error: `PowerPoint export failed: ${message}` },
      { status: 500 }
    );
  }
}
